...
public class computationImpl extends SqlMapClientDaoSupport implements computationDao {
	...
	public String runComputation(ReportInfoGetter input) {
		...
		try {
			...
			if (li.size() > 0) {
				...
				for (ReportInfoGetter entity : li) {
					...
					if (!(lastentity.getRegistration_number().equalsIgnoreCase(entity.getRegistration_number()))) {
						...
						/*
						 * not required ReportInfoGetter getGenderEliibility =
						 * (ReportInfoGetter)client.queryForObject("GenderEliibility",entityInfo); //Q5
						 * if ((getGenderEliibility.getStatus().equalsIgnoreCase("InEligible"))&&(
						 * getGenderEliibility.getReason_code().equalsIgnoreCase("GenderEligible"))) {
						...
						Integer count = (Integer) getSqlMapClientTemplate()
								.queryForObject("computation.checkDuplicateForComputeMarks", entityInfo);
						if (count > 0) {
							...
							List<ReportInfoGetter> FMC_CompInsert = getSqlMapClientTemplate()
									.queryForList("computation.checkFMCInsert", entityInfo); // Q8
							if (FMC_CompInsert.size() == 0) {
								ReportInfoGetter RollNumber = (ReportInfoGetter) getSqlMapClientTemplate()
										.queryForObject("computation.checkFMCInsert2", entityInfo); // Q9
					...
					System.out.println(
							"Computation Logic" + entityInfo.getRegistration_number() + "-" + entity.getComponent_id());
					...
					ReportInfoGetter roundoffmarks = (ReportInfoGetter) getSqlMapClientTemplate()
							.queryForObject("computation.getComputed_marks_roundoff", entityInfo); // Q11
			...
			getSqlMapClientTemplate().update("computation.updatefinalscore_AS", entityInfo); // Q18 // program_component
																								// group wise computed
																								// marks total from
																								// (SCL) into
																								// academic_score coloum
																								// in (STN)
			System.out.println("updatefinalscore_AS"); // added by atul dayal 06-07-2016

			getSqlMapClientTemplate().update("computation.updatebestscore", entityInfo); // Q19 best between between
																							// sum_computed_marks_old
																							// and
																							// sum_computed_marks_roundoff
			System.out.println("updatebestscore");

			getSqlMapClientTemplate().update("computation.updateET_AS_score", entityInfo); // Q20 add sum_computed_score
																							// amd entrance score and
																							// uptate it into (STN)
																							// best_score column
	...
	private ReportInfoGetter getcomponenteligibility1(ReportInfoGetter entity) throws Exception {
		 ...
		 * List<ReportInfoGetter> ageeligiblity =
		 * client.queryForList("computation.getAgeEligibility",entity); if
		 * (ageeligiblity.size()>0){ String eligibility = null;System.out.println(
		 * "Computing Marks For : "+ entity.getRegistration_number()+
		 * " : "+entity.getGender());
		 * if(entity.getGender().trim().equalsIgnoreCase("M")) { eligibility =
		 * getAgeEligibility(entity,ageeligiblity, entity.getRegistration_number()); }
		 * else { eligibility = "Eligible"; }
		 * 
		 * if (eligibility.equalsIgnoreCase("InEligible")) {
		 * obj.setReason_code("Overage"); obj.setMessage("InEligible"); return obj;
		 * 
		 * } }
		 */
		///// age Eligibility End/////////////////////////////////////
		...
		List<ReportInfoGetter> agregateeligiblity = getSqlMapClientTemplate()
				.queryForList("computation.getStudentPercentage", entity);
		if (agregateeligiblity.size() > 0) {
			String eligibility = null;

			if (Double.parseDouble(agregateeligiblity.get(0).getAgregate_percentage()) >= Double
					.parseDouble(agregateeligiblity.get(0).getMinimum_agregate())) {
				obj.setReason_code("You are Eligible");
				obj.setMessage("Eligible");
		...
		compulasryEligibility = getSqlMapClientTemplate().queryForList("computation.get_compulsary_eligibility",
				entity);
		...
		if (compulasryEligibility.size() > 0) {
			...
			for (ReportInfoGetter ce : compulasryEligibility) {
				...
				if (ce.getMarks_percentage() >= ceeligibility) {
					...
					obj.setReason_code("COM:"
							+ getComponentDescription(ce.getComponent_id(), ce.getRegistration_number(), entity) + " < "
	...
	public String getComponentDescription(String component_id, String regnum, ReportInfoGetter entityInfo) {
		...
		try {
			...
			// logObj.logger.info((new StringBuilder("Error while getting Component
			// Descrition for : ")).append(component_id).append(",
			// registration").append(regnum).toString());
			// logObj.logger.info((new StringBuilder("Exception is:
	...
	public double getComponetEligibleMarks(String component_id, String registration_number,
		...
		try {
			...
			// logObj.logger.info((new StringBuilder("Error while getting eligible for
			// component marks for ")).append(registration_number).toString());
		}
		return marks;
	}
	...
	private void magicmethod(String program_id) {
		...
		try {
			ReportInfoGetter cutoffupdate = (ReportInfoGetter) getSqlMapClientTemplate()
					.queryForObject("computation.getcalledapplicants", enInfo);
			...
			if (cutoffupdate.getNumber().equalsIgnoreCase("4")) {
				...
				try {
					getSqlMapClientTemplate().update("computation.updateCalledApplicants_cutoff_based_on_Category",
							enInfo);
				...
				System.out.println(
						"Inside :updateCalledApplicants_cutoff_based_on_Category_Gender" + enInfo.getProgram_id());

				try {

					getSqlMapClientTemplate()
							.update("computation.updateCalledApplicants_cutoff_based_on_Category_Gender", enInfo);
	...
	private void EnteranceTestEligibility(String Program_id) {
		ReportInfoGetter enInfo = new ReportInfoGetter();
		try {
			ReportInfoGetter getCatCutOff = (ReportInfoGetter) getSqlMapClientTemplate()
					.queryForObject("computation.getCatCutOff", enInfo);
			...
			getSqlMapClientTemplate().update("computation.updateCalledApplicants_BlankET", enInfo); // added by atul
																									// dayal 06-07-2016
			getSqlMapClientTemplate().update("computation.updateCalledApplicants_AbsentInET", enInfo); // added by atul
																										// dayal
																										// 09-08-2020
	...
	public String finalMeritListProcessforAll(ReportInfoGetter input) {
		...
		try {
			...
			for (int l1 = 0; l1 < programList.size(); l1++) {
				...
				List<ReportInfoGetter> regList = getSqlMapClientTemplate()
						.queryForList("computation.getRegisteredStudent", reportBeen);
				System.out.println("regList:" + regList.size());
				// client.update("computation.updateStudentFinalMarksforMerit",reportBeen);
				...
				if (regList.size() > 0) {
					...
					for (int i = 0; i < regList.size(); i++) {
						totalscore = 0;
						reportBeen.setRegistration_number(regList.get(i).getRegistration_number());
						// List<ReportInfoGetter>compList=client.queryForList("computation.getComponentDetail",
						// reportBeen);

						List<ReportInfoGetter> Academicscore = getSqlMapClientTemplate()
								.queryForList("computation.getAcademicScoreAll", reportBeen);
						List<ReportInfoGetter> NonAcademicscore = getSqlMapClientTemplate()
								.queryForList("computation.getNonAcademicScore", reportBeen);
			...
			message = "Total applicants:" + String.valueOf(totalStudents) + " processed ." + "Total records:"
					+ String.valueOf(recordProcessed) + " Processed";
		} catch (Exception e) {
			System.out.println(e);
			message = e.toString();
		}
...

	{
		...
		if (input.getFlag().equalsIgnoreCase("A")) {
			...
			if (semDates.size() > 0) {
				...
				getData = getSqlMapClientTemplate().queryForList("computation.getCourseInfoForTTforEditpckWiseOld",
						input);
			}
			System.out.println(input.getSemester_start_date());
			System.out.println(input.getSemester_end_date());
			...
			List<admissionBean> checkData = getSqlMapClientTemplate().queryForList("computation.checkRoomCourseMapping",
					input);
			...
			List<admissionBean> checkData = getSqlMapClientTemplate()
					.queryForList("computation.checkTeacherCourseMapping", input);
			...
			List<admissionBean> checkData = getSqlMapClientTemplate()
					.queryForList("computation.checkTimeTableCourseInfo", input);
			...
			System.out.println(input.getSection() + "--" + input.getSectionType() + "--" + input.getGender() + "--"
					+ input.getCapacity());
			...
			List<admissionBean> checkData = getSqlMapClientTemplate()
					.queryForList("computation.checkTimeTableCourseSection", input);
			if (checkData.size() == 0) {
			...
			List<admissionBean> checkData = getSqlMapClientTemplate()
					.queryForList("computation.checkTimeTableJointlecture", input);
			if (checkData.size() == 0) {
				// getSqlMapClientTemplate().delete("computation.DeleteTimeTableJointlecture",
				// input);
...
public String runComputationforAll(ReportInfoGetter input,String mode) {
	 ...
	 //computation starts!
		 ...
		 String program=programWise.get(PI).getProgram_id();
				...
				System.out.println("Computation Logic"+entityInfo.getRegistration_number()+"-"+entity.getComponent_id());
				 ...
				 double boardPercentage = 0.0;
					 ...
					 else
					 {
               		...
               		// Arush for Btech JEE score on 06-06-2025
					
					 if (entity.getComponent_id().equalsIgnoreCase("JM") && program.equalsIgnoreCase("0001067")) {
						ccaBean ccabean= jeeMarks(registrationNumber);
						if (ccabean.getMarks()!=null) {
							
						
						entityInfo.setComputed_Marks(Double.parseDouble(ccabean.getMarks()));
						entityInfo.setActual_computed_Marks(Double.parseDouble(ccabean.getMarks()));
						}
						else
							entityInfo.setComputed_Marks(0);
						    entityInfo.setActual_computed_Marks(0);
	 ...
	 * 
	 * 
	 * transferBean transferstudent= new transferBean(); transferBean inp1= new
	 * transferBean();
	 * 
	 * List<transferBean> transferProgramList= new ArrayList<transferBean>(); List
	 * <transferBean> paperoption1 =new ArrayList<transferBean>(); List <String>
	 * COURSES =new ArrayList<String>(); List <String> Distinct =new
	 * ArrayList<String>(); List <String> pap1 =new ArrayList<String>(); List
	 * <String> pap2 =new ArrayList<String>(); List <String> pap3 =new
	 * ArrayList<String>(); List <String> pap4 =new ArrayList<String>(); List
	 * <String> pap5 =new ArrayList<String>(); List <String> programList =new
	 * ArrayList<String>(); String pasdob;
	 * 
	 * 
	 * transferstudent = (transferBean)getSqlMapClientTemplate().queryForObject(
	 * "transfer.checkfortransferapp",input);
	 ...
	 * 
	 * 
	 * BubbleFormBean EntitySession = (BubbleFormBean)
	 * getSqlMapClientTemplate().queryForObject("bubbleform.EntitySession");
	 ...
	 * 
	 * pasdob=transferstudent.getDob().substring(0, 4); String na1=
	 * transferstudent.getCandidateName().substring(0,3); String asd =na1+pasdob;
	 * transferstudent.setPassword(asd);
	 * 
	 * 
	 * transferstudent.setFee("0"); transferstudent.setUniversityId("0001");
	 ...
	 * checkApplication
	 * =getSqlMapClientTemplate().queryForList("transfer.checkApplication",
	 * transferstudent);
	 * 
	 ...
	 * try {
	 * 
	 * input.setUgpg("STUDID"); input .setUniversityId("0001");
	 ...
	 * 
	 * getSqlMapClientTemplate().insert("bubbleform.setApplicantAccountInfo",
	 * transferstudent);
	 * getSqlMapClientTemplate().insert("bubbleform.setEntityStudentUserform",
	 * transferstudent); getSqlMapClientTemplate().insert(
	 * "bubbleform.setStudentAddressUserFormtransfer", transferstudent);
	 * getSqlMapClientTemplate().insert(
	 * "bubbleform.setStudentAddressUserFormtransferForPER", transferstudent);
	 * 
	 * 
	 * System.out.println(programList.size());
	 * 
	 * for (int k=0 ; k<programList.size();k++) { String programId=
	 * programList.get(k).substring(0,7); transferstudent.setProgramName(programId);
	 * 
	 * System.out.println(k+"-"+programId);
	 * 
	 * 
	 * 
	 * transferBean getentity = (transferBean)
	 * getSqlMapClientTemplate().queryForObject("transfer.getentityid",
	 * transferstudent);
	 * 
	 ...
	 * 
	 * input.setUgpg("APPNUM"); input=getregistrationnumber(input);
	 ...
	 * 
	 * 
	 * getSqlMapClientTemplate().insert("bubbleform.setStudentRegistrationUserForm",
	 * transferstudent); getSqlMapClientTemplate().insert(
	 * "bubbleform.setApplicantProgramRegistrationUserForm",transferstudent);
	 * getSqlMapClientTemplate().insert("bubbleform.insertIntoStudentFinalMarks",
	 * transferstudent);
	 * getSqlMapClientTemplate().insert("bubbleform.setStudenttestNumberUserForm",
	 * transferstudent);
	 ...
	 * );
	 * 
	 * System.out.println("*****"+transferstudent.getActualApplication()+"-"+
	 * transferstudent.getFromprogram());
	 * System.out.println(transferstudent.getAppnumber()+"-"+transferstudent.
	 * getProgram_id());
	 * 
	 * 
	 ...
	 * 
	 * for (int y=0 ; y<pap2.size();y++) { System.out.println("a2");
	 * transferstudent.setProgram_id(pap2.get(y));
	 * System.out.println("X0-"+transferstudent.getProgram_id()); transferBean
	 * getTempAppNumber = (transferBean)
	 * getSqlMapClientTemplate().queryForObject("transfer.getTempAppNumber",
	 * transferstudent);
	 * System.out.println("X1"+getTempAppNumber.getTempApplicationNumber());
	 * transferstudent.setTempApplicationNumber(getTempAppNumber.
	 * getTempApplicationNumber());
	 * System.out.println("tempApp-"+getTempAppNumber.getTempApplicationNumber()+
	 * "--"+transferstudent.getProgram_id());
	 * 
	 * List<transferBean> checkPaper = new ArrayList<transferBean>(); checkPaper
	 * =getSqlMapClientTemplate().queryForList("transfer.checkPaper",transferstudent
	 * );
	 * 
	 * if (checkPaper.size()>0) {
	 * getSqlMapClientTemplate().insert("transfer.setDefaultPaperCodes",
	 * transferstudent); } else {
	 * getSqlMapClientTemplate().insert("transfer.setDefaultPaperCodes2",
	 * transferstudent); }
	 * 
	 * 
	 * System.out.println("a3"); } pap2.clear();
	 * 
	 * 
	 ...
	 * 
	 * getSqlMapClientTemplate().insert(
	 * "bubbleform.setStudentApplicationStatusUserFormTrasfer",transferstudent);
	 * getSqlMapClientTemplate().update("bubbleform.updateStudentApplicationStatus",
	 * transferstudent);
	 * 
	 * transferstudent.setStatus("OK"); return transferstudent;
	 * 
	 * 
	 * } catch (Exception e) { System.out.println("erroeI--"+e);
	 * transferstudent.setStatus(e.toString()); } } else {
	 * transferstudent.setStatus("ALL_Ready_Transfered"); } } catch(Exception e) {
	 * 
	 ...
	 * transferBean();
	 * 
	 * transferBean abc = (transferBean)transactionTemplate.execute(new
	 * TransactionCallback() { //Object savePoint ;
	 * 
	 * public Object doInTransaction(TransactionStatus ts) {
	 * 
	 * 
	 * Object savePoint= new Object(); try { System.out.println("Don"); savePoint =
	 * ts.createSavepoint(); } catch (Exception e) {
	...
	public transferBean transferApp(final transferBean input) {
		...
		transactionTemplate.execute(new TransactionCallback() {
			...
			public Object doInTransaction(TransactionStatus ts) {
				...
				transferstudent = (transferBean) getSqlMapClientTemplate()
						.queryForObject("transfer.checkfortransferapp", input);
				transferstudent.setTransferFlag(input.getTransferFlag());
				transferstudent.setAppnumber(input.getTransferapp());
				transferstudent.setFromprogram(input.getFromprogram());
				transferstudent.setActualApplication(input.getActualApplication());
				...
				BubbleFormBean EntitySession = (BubbleFormBean) getSqlMapClientTemplate()
						.queryForObject("bubbleform.EntitySession");
				transferstudent.setSession(EntitySession.getSession());
				transferstudent.setStartdate(EntitySession.getStartdate());
				transferstudent.setEnddate(EntitySession.getEnddate());
				...
				try {
					...
					List<transferBean> checkApplication = new ArrayList<transferBean>();
					checkApplication = getSqlMapClientTemplate().queryForList("transfer.checkApplication",
							transferstudent);
					...
					if (checkApplication.size() == 0) {
						...
						try {
							...
							getSqlMapClientTemplate().insert("bubbleform.setStudentAddressUserFormtransfer",
									transferstudent);
							getSqlMapClientTemplate().insert("bubbleform.setStudentAddressUserFormtransferForPER",
									transferstudent);
							...
							for (int k = 0; k < programList.size(); k++) {
								...
								transferBean getentity = (transferBean) getSqlMapClientTemplate()
										.queryForObject("transfer.getentityid", transferstudent);
								...
								getSqlMapClientTemplate().insert("bubbleform.setStudentRegistrationUserForm",
										transferstudent);
								getSqlMapClientTemplate().insert("bubbleform.setApplicantProgramRegistrationUserForm",
										transferstudent);
								getSqlMapClientTemplate().insert("bubbleform.insertIntoStudentFinalMarks",
										transferstudent);
								getSqlMapClientTemplate().insert("bubbleform.setStudenttestNumberUserForm",
								...
								System.out.println("*****" + transferstudent.getActualApplication() + "-"
										+ transferstudent.getFromprogram());
								System.out.println(
										transferstudent.getAppnumber() + "-" + transferstudent.getProgram_id());
								...
								if (input.getPaperOptionFlag1().equalsIgnoreCase("D")) {
									...
									for (int y = 0; y < pap2.size(); y++) {
										...
										// transferBean getTempAppNumber = (transferBean)
										// getSqlMapClientTemplate().queryForObject("transfer.getTempAppNumber",transferstudent);
										List<transferBean> getTempAppNumber = getSqlMapClientTemplate()
												.queryForList("transfer.getTempAppNumber", transferstudent);

										if (getTempAppNumber.size() > 0) {
											System.out
													.println("X1" + getTempAppNumber.get(0).getTempApplicationNumber());
											transferstudent.setTempApplicationNumber(
													getTempAppNumber.get(0).getTempApplicationNumber());
											System.out.println(
													"tempApp-" + getTempAppNumber.get(0).getTempApplicationNumber()
															+ "--" + getTempAppNumber.get(0).getProgram_id());
										} else {
											transferstudent.setTempApplicationNumber(" ");
										}

										List<transferBean> checkPaper = new ArrayList<transferBean>();
										checkPaper = getSqlMapClientTemplate().queryForList("transfer.checkPaper",
												transferstudent);
							...
							getSqlMapClientTemplate().insert("bubbleform.setStudentApplicationStatusUserFormTrasfer",
									transferstudent);
							getSqlMapClientTemplate().update("bubbleform.updateStudentApplicationStatus",
									transferstudent);
	...
	private transferBean getregistrationnumber(transferBean inputbean) {
		...
		try {
			...
			transferBean sysValueObj = (transferBean) getSqlMapClientTemplate()
					.queryForObject("transfer.getSystemValues", inputbean);
	...
	public admissionBean UpdateSCLMarks(final admissionBean input) {
		...
		vfystatus = (admissionBean) getSqlMapClientTemplate().queryForObject("computation.getverificationstatus",
				input);
...
// Changes By Arush on 02-08-2021 to distribute Marks
	...
	public String distributETMarks() {
		...
		for (ReportInfoGetter applicant : applicants) {
			...
			try {
				...
				List<ReportInfoGetter> commonpgms = getSqlMapClientTemplate()
						.queryForList("computation.getcommonprograms", input);
	...
	public void distributemarks(String commonpgmObj, ReportInfoGetter applicant, List<String> bindedapps, String flag)
			throws SQLException {
...
//	}
		...
		for (ReportInfoGetter appl : result) {
			...
			if (checklist.size() > 0) {
				System.out.println(
						"Application alreadu exists in comp_marks_dist\n Application Number: " + input.getTest_number()
								+ "Program_id" + input.getProgram_id() + "bindedApplication:" + bindedapps);
			} else {
...
// Added by Manpreet on 02-06-2025
	...
	public ccaBean jeeMarks(String regno) {
		ccaBean bean = new ccaBean();
		bean.setRegistration_number(regno);
		List<ccaBean> getApplication = getSqlMapClientTemplate().queryForList("jee.GetJEEcandidates",bean);
		List<ccaBean> getDataforUpdate = null;
		for (int i = 0; i < getApplication.size(); i++) {
			bean.setMarksObtained(getApplication.get(i).getMarksObtained());
			...
			//if (checkDatainJEEStudent.size() > 0) {
				...
				if (getDataforUpdate.size() == 0) {
					...
					bean.setMarksObtained(Double.toString(marks));

					getDataforUpdate = getSqlMapClientTemplate().queryForList("jee.getDataforUpdate", bean);

					if (getDataforUpdate.size() == 0) {
						double marks1 = Double.parseDouble(bean.getMarksObtained()) + 0.6;
						bean.setMarksObtained(Double.toString(marks1));

						getDataforUpdate = getSqlMapClientTemplate().queryForList("jee.getDataforUpdate", bean);

						if (getDataforUpdate.size() == 0) {

							double marks2 = Double.parseDouble(bean.getMarksObtained()) + 1.1;
							bean.setMarksObtained(Double.toString(marks2));
							...
							if (getDataforUpdate.size() == 0) {
								...
								bean.setMarks(getDataforUpdate.get(0).getMarks());
							}
						} else {
							bean.setMarks(getDataforUpdate.get(0).getMarks());
						}
